...
# Тегеран Таймс и Мехр переехали в добавляемые RSS-ленты (2026-09-06);
# осталась Аль-Масира — у неё нет RSS-канала, поэтому только вкл/выкл.
ЛЕНТЫ_ИМЕНА = {
...
def записать_расписание(часы, дни):
    ...
    for ч in часы:
        if not str(ч).isdigit():
            continue
        h = int(ч)
        if h == 24:                     # полночь часто пишут как 24
            h = 0
        if 0 <= h <= 23:
            норм.append(h)
    часы_и = sorted(set(норм)) or [6]
    дни = [д for д in дни if str(д).isdigit()] or ["2", "3", "4", "5", "6"]
    строка_часов = ",".join(str(h) for h in часы_и)
    ...
    # первый выпуск минус 10 минут — поднять голос Fish (с переносом через полночь)
    первый = min(часы_и)
    рыба = (f"50 {(первый - 1) % 24} * * {строка_дней} "
...
def статистика_html():
    ...
    return (
        ...
        + перечень_html("без_звука", "Скачался без звука (нет звуковой дорожки)")
        + перечень_html("задушен", "Задушен ютубом (придушил скорость)")
        + перечень_html("не_скачалось", "Не удалось скачать (403/сеть/иное)")
...
def сохранить_расписание(поля):
    ...
    ok, часы_норм = записать_расписание(часы, дни)
    if not ok:
        return ("не смог записать расписание — проверь часы: допустимы "
                "0–23, полночь пиши 0 или 24 (не 25 и выше)")
    имена = ", ".join(dict(ДНИ)[д] for д in sorted(set(дни), key=int)
                      if д in dict(ДНИ))
    return (f"расписание сохранено: {','.join(str(h) for h in часы_норм)} ч, "
            f"дни {имена}")
...
def добавить_канал(поля):
    ...
    m = (re.search(r"@([\w.\-]+)", сырьё)
         or re.search(r"youtube\.com/(?:c/|channel/|user/)?([\w.\-]+)",
                      сырьё, re.I))
    ...
    for c in cfg.get("ютуб", []):
        if хл == (re.search(r"@([\w.\-]+)", c.get("url", "")) or
                  [None, ""])[1].lower():
